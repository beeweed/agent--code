...
export const E2BSandboxWrapper: React.FC<E2BSandboxProps> = ({ chatStarted }) => {
  ...
  useEffect(() => {
    if (sandboxError) {
      e2bStore.setError(sandboxError);
    }
  }, [sandboxError]);

  // Register callbacks with the store when sandbox is connected
  useEffect(() => {
    if (isConnected) {
      const runCommandInTerminal = async (command: string) => {
        if (activeTerminalIdState) {
          // Send command to terminal followed by Enter
          await sendTerminalInput(activeTerminalIdState, command + '\n');
        } else {
      ...
      e2bStore.registerSandboxCallbacks({
        writeFile,
        makeDirectory,
        runCommand: runCommandInTerminal,
        sendTerminalInput,
        createSandbox,
        setActiveTerminalId: (id: string) => setActiveTerminalIdState(id),
        getActiveTerminalId: () => activeTerminalIdState,
      });
  ...
  useEffect(() => {
    const unsubscribe = e2bApiKey.subscribe((newKey) => {
      if (newKey && newKey !== internalApiKey) {
        setInternalApiKey(newKey);
      }
    });
    return () => unsubscribe();
  }, [internalApiKey, setInternalApiKey]);
...
export { useFileSystem, useE2BSandbox };
